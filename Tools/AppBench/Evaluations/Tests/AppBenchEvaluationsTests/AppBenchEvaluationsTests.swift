...
final class AppBenchEvaluationsTests: XCTestCase {
  ...
  func testLoadsCurrentAppBenchResult() throws {
    let run = makeCurrentRun()
    let data = try XCTUnwrap(
      AppBenchReport(result: run).json().data(using: .utf8)
    )

    let recorded = try AppBenchRecordedRunLoader.decode(
      data,
    ...
    let run = AppBenchRecordedRun(
      ...
      records: [
        AppBenchEvaluationRecord(
          ...
          response: "Done",
          toolCalls: [
            AppBenchToolCall(name: "lookup", arguments: [:])
          ]
        ),
        AppBenchEvaluationRecord(
          scenarioID: "plain",
