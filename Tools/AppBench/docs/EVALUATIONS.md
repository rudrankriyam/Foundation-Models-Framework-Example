# AppBench and Apple Evaluations

AppBench uses Apple’s Evaluations framework as a **macOS 27 evaluation control
...
1. `AppBenchCore` runs deployment-shaped workloads and records portable JSON.
2. The macOS-only `AppBenchEvaluations` package replays those recorded responses
   through deterministic Evaluations metrics.
3. The AppBench-specific `appbench-evaluate` command creates native
   `.xcevalresult` files.
...
original AppBench trial.

Apple makes Evaluations available on macOS, iOS, watchOS, and visionOS 27, with
tvOS unavailable. AppBench’s macOS-only boundary is an intentional product and
tooling decision, not a claim that Apple lacks other platform binaries. The
shipping iOS device runner continues to use only `AppBenchCore`.
...
AppBench runner
  |
  | portable AppBench JSON
  v
appbench-evaluate replay (macOS 27)
  |
  | Evaluation.run(info:)
  v
.xcevalresult JSON
...
| `Xcode.app/Contents/SharedFrameworks/MLEvaluation*.framework` | Other private/legacy ML evaluation support | No |
...
The AppBench package manifest adds the developer framework search path and an Xcode
`Contents` runpath. `Tools/AppBench/appbench-evaluate` discovers Xcode installations
...
AppBench currently uses deterministic evaluators and native tool trajectory
...
AppBench owns only the producer command because it understands AppBench’s portable
result schema and deterministic graders:

```bash
# Convert a recorded AppBench run into a native evaluation result.
...
- Execution success.
- Deterministic AppBench prompt pass.
- Deterministic AppBench constraint score.
