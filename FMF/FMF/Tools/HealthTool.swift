...
struct HealthTool: Tool {
    ...
    private func queryHeartRate(arguments: Arguments) async -> ToolOutput {
        ...
        let (startDate, endDate) = getDateRange(arguments: arguments)
        let datePredicate = HKQuery.predicateForSamples(withStart: startDate, end: endDate, options: .strictStartDate)
        let samplePredicate = HKSamplePredicate.quantitySample(type: heartRateType, predicate: datePredicate)
    ...
    private func querySleep(arguments: Arguments) async -> ToolOutput {
        ...
        let (startDate, endDate) = getDateRange(arguments: arguments)
        let datePredicate = HKQuery.predicateForSamples(withStart: startDate, end: endDate, options: .strictStartDate)
        let samplePredicate = HKSamplePredicate.categorySample(type: sleepType, predicate: datePredicate)
    ...
    private func queryActiveEnergy(arguments: Arguments) async -> ToolOutput {
        ...
        let (startDate, endDate) = getDateRange(arguments: arguments)
        let datePredicate = HKQuery.predicateForSamples(withStart: startDate, end: endDate, options: .strictStartDate)
        let samplePredicate = HKSamplePredicate.quantitySample(type: energyType, predicate: datePredicate)
    ...
    private func queryDistance(arguments: Arguments) async -> ToolOutput {
        ...
        let (startDate, endDate) = getDateRange(arguments: arguments)
        let datePredicate = HKQuery.predicateForSamples(withStart: startDate, end: endDate, options: .strictStartDate)
        let samplePredicate = HKSamplePredicate.quantitySample(type: distanceType, predicate: datePredicate)
