...
actor AppBenchToolRecorder {
    private var calls: [AppBenchToolCall] = []

    func record(_ call: AppBenchToolCall) {
        calls.append(call)
    }
    ...
    func snapshot() -> [AppBenchToolCall] {
        calls
    }
}
...
struct AppBenchSessionBundle: Sendable {
    let session: LanguageModelSession
    let recorder: AppBenchToolRecorder
}

func appBenchTools(
    for toolSet: AppBenchToolSet,
    recorder: AppBenchToolRecorder
) -> [any Tool] {
    switch toolSet {
    case .none:
        []
