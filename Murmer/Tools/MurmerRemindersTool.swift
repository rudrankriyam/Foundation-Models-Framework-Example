...
struct MurmerRemindersTool: Tool {
    ...
    func call(arguments: Arguments) async throws -> ToolOutput {
        ...
        if let timeExpression = arguments.timeExpression {
            ...
            if let dueDate = parseTimeExpression(timeExpression) {
                ...
                print("[MurmerRemindersTool] Set due date components: year=\(components.year ?? -1), month=\(components.month ?? -1), day=\(components.day ?? -1), hour=\(components.hour ?? -1), minute=\(components.minute ?? -1)")
        ...
        if let listName = arguments.listName {
            ...
            do {
                ...
                print("[MurmerRemindersTool] ERROR: Failed to get/create list: \(error.localizedDescription)")
                throw error
            }
        } else {
    ...
    private func parseTimeExpression(_ expression: String) -> Date? {
        ...
        if lowercased.contains("tomorrow") {
            ...
            if let time = extractTime(from: lowercased) {
                print("[MurmerRemindersTool.parseTimeExpression] Extracted time: \(time.hour):\(time.minute)")
            ...
            print("[MurmerRemindersTool.parseTimeExpression] Tomorrow date calculated: \(result?.description ?? "nil")")
            return result
        }
    ...
    private func extractTime(from text: String) -> (hour: Int, minute: Int)? {
        ...
        print("[MurmerRemindersTool.extractTime] Using patterns: \(patterns)")
        
        for (index, pattern) in patterns.enumerated() {
            print("[MurmerRemindersTool.extractTime] Trying pattern \(index): \(pattern)")
            do {
                    ...
                    if match.numberOfRanges >= 2,
                       ...
                       let hour = Int(text[hourRange]) {
                        ...
                        if match.numberOfRanges >= 3 {
                            ...
                            print("[MurmerRemindersTool.extractTime] Checking for AM/PM at index \(amPmIndex)")
